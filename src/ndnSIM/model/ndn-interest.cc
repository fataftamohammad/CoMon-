...
Interest::Interest (const Interest &interest)
  ...
  , m_monitored        (interest.m_monitored)
  , m_served           (interest.m_served)
...
void
Interest::SetMonitored (uint32_t monitored)
{
...
uint32_t
Interest::GetMonitored () const
{
  return m_monitored;
}

void
Interest::SetServed (uint32_t served)
{
  m_served = served;
  m_wire = 0;
}
