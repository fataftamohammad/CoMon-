...
Face::Send (Ptr<Packet> packet)
{
  ...
  // if (tagExists)
  //   {
  //     if(hopCount.GetHops().back() != GetNode()->GetId())
  //     {
  //         // Dont increment the hopcount if the last hop was already me. That can happen if an
  //         // interest is forwarded between different apps on one node.
  //         // hopCount.Increment ();
  //         // hopCount.Add(GetNode()->GetId());
  //     }
